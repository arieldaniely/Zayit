...
object PersonalLibraryCatalogMerger {
    ...
    internal fun merge(
            ...
            connection.createStatement().use { statement ->
                statement
                    .executeQuery(
                        ...
                        GROUP BY b.id ORDER BY b.orderIndex,b.title
                        """.trimIndent(),
                    ).use { rows ->
                        while (rows.next()) {
                            books +=
                                CatalogBook(
                                    ...
                                    categoryId = rows.getLong(3),
                                    order = rows.getFloat(4),
                                    totalLines = rows.getInt(5),
                                    isBaseBook = rows.getInt(6) != 0,
                                    hasTargumConnection = rows.getInt(7) != 0,
                                    hasReferenceConnection = rows.getInt(8) != 0,
                                    hasSourceConnection = rows.getInt(9) != 0,
                                    hasCommentaryConnection = rows.getInt(10) != 0,
                                    hasOtherConnection = rows.getInt(11) != 0,
                                    hasAltStructures = rows.getInt(12) != 0,
                                    authors =
                                        rows
                                            .getString(13)
                                            ?.split('|')
        ...
        fun personalNode(row: CategoryRow): CatalogCategory =
            CatalogCategory(
                id = row.id,
                title = row.title,
                level = row.level,
        ...
        fun augment(node: CatalogCategory): CatalogCategory =
            node.copy(
